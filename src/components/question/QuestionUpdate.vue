      ...
      <a-form :model="form">
        ...
        <a-form-item label="题目模板" :content-flex="false" :merge-props="false">
          ...
          <div style="display: flex;align-items: end">
            <a-button type="primary" @click="addModelFlag" style="margin-left: auto;">编辑模板</a-button>
          </div>
          <a-modal v-if="codeModel" v-model:visible="codeModel" title="修改模板" @ok="addTemplate"
                   @cancel="codeModel = false">
            <CodeEditorVue :on-change="changeCode" :on-select="changeLanguage" :value="{
              language: 'json',
              code: JSON.stringify(form.questionTemplates)
            }" class="editor"
        ...
        <a-form-item label="判题用例" :content-flex="false" :merge-props="false">
          ...
          <div style="display: flex;align-items: end">
            <a-button type="primary" @click="addCaseModelFlag" style="margin-left: auto;">编辑用例</a-button>
          </div>
          <a-modal v-if="caseModel" v-model:visible="caseModel" title="编辑用例（JSON格式）" @ok="addJudgeCase"
                   @cancel="caseModel = false">
            <CodeEditorVue :on-change="changeCase" class="editor"
                           :value="{
              language: 'json',
              code: JSON.stringify(form.judgeCase)
                           }"
                           :height="'200px'"></CodeEditorVue>
          </a-modal>
        </a-form-item>
        <a-form-item label="测试判题用例" :content-flex="false" :merge-props="false">
          ...
          <div style="display: flex;align-items: end">
            <a-button type="primary" @click="addTestCaseModelFlag" style="margin-left: auto;">编辑测试用例</a-button>
          </div>
          <a-modal v-if="testCaseModel" v-model:visible="testCaseModel" title="编辑测试用例（JSON格式）"
                   @ok="addTestJudgeCase"
                   @cancel="testCaseModel = false">
            <CodeEditorVue :on-change="changeTestCase" class="editor"
                           :value="{
              language: 'json',
              code: JSON.stringify(form.testJudgeCase)
                            } "
                           :height="'200px'"></CodeEditorVue>
          </a-modal>
        </a-form-item>
        ...
        <a-button type="primary" status="success" @click="submit"
                  style="margin-left: auto; border-radius: 6px; width: 150px">提交
        </a-button>
      </a-form>
    </div>
  </div>
